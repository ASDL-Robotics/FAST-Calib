...
``/fast_calib/debug/pointcloud``. Also publishes a wireframe box marker
showing the filter bounds on ``/fast_calib/debug/filter_box``.

If camera images exist for the scene, they are published on
``/fast_calib/debug/<camera_name>/image``.
...
class DatasetPublisher(Node):
    ...
    ----------
    dataset_name:
        Human-readable name for this dataset (used in log messages).
    bag_path:
    ...
    def __init__(
        ...
        self._cloud_topic = f'{_TOPIC_PREFIX}/pointcloud'
        self._cloud_pub = self.create_publisher(
            PointCloud2, self._cloud_topic, _DEBUG_QOS
        )

        # Filter box marker publisher.
        self._box_marker_pub = self.create_publisher(
            Marker, f'{_TOPIC_PREFIX}/filter_box', _DEBUG_QOS
        )
    ...
    def _publish_filter_box(self) -> None:
        ...
        marker = _make_filter_box_marker(
            self.filter_bounds, frame_id, f'{_TOPIC_PREFIX}/filter_box'
        )
        marker.header.stamp = self.get_clock().now().to_msg()
        self._box_marker_pub.publish(marker)
        self.get_logger().info(f'Published filter box marker on {_TOPIC_PREFIX}/filter_box')
    ...
    def _publish_images(self) -> None:
        ...
        for camera_name, img_path in self.image_paths:
            ...
            topic = f'{_TOPIC_PREFIX}/{camera_name}/image'
...
def publish_dataset(
    ...
    ----------
    dataset_name:
        Name used in log messages to identify which dataset is being replayed.
    bag_path:
