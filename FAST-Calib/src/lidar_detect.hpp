...
public:
    ...
    void detect_mech_lidar(pcl::PointCloud<Common::Point>::Ptr cloud, pcl::PointCloud<pcl::PointXYZ>::Ptr center_cloud)
    {
        ...
        // 2. Plane fitting — extract normal vector
        plane_cloud_->clear();
        ...
        // 3. Extract edge points using per-ring neighbor distance jumps
        edge_cloud_->clear();
        ...
        // Detect edge points as neighbor-distance jumps within each ring
        ...
        RCLCPP_INFO(node_->get_logger(), "Extracted %zu edge points (mechanical LiDAR by neighbor distance).", edge_cloud_->size());
        ...
        // 5. Detect circles in the aligned cloud using iterative RANSAC
        center_z0_cloud_->clear();
        pcl::PointCloud<pcl::PointXYZ>::Ptr xy_cloud(new pcl::PointCloud<pcl::PointXYZ>(*aligned_cloud_));

        RCLCPP_INFO(node_->get_logger(), "[LiDAR] Start circle detection, initial cloud size = %zu", xy_cloud->points.size());
        ...
        {
            RCLCPP_INFO(node_->get_logger(), "[LiDAR] RANSAC on cloud of size %lu", xy_cloud->points.size());
            ...
            {
                RCLCPP_INFO(node_->get_logger(), "[LiDAR] Found circle but inliers too few (%zu < 5), stop.",
        ...
        // 7. Inverse-transform selected circle centers back to original frame
    ...
    void detect_solid_lidar(pcl::PointCloud<Common::Point>::Ptr cloud, pcl::PointCloud<pcl::PointXYZ>::Ptr center_cloud)
    {
        ...
        RCLCPP_INFO(node_->get_logger(), "Filtered cloud size after voxel: %zu", filtered_cloud_->size());
        ...
        // 4. Extract boundary (edge) points
        edge_cloud_->clear();
        ...
        RCLCPP_INFO(node_->get_logger(), "Extracted %zu edge points.", edge_cloud_->size());
        ...
        RCLCPP_INFO(node_->get_logger(), "Number of edge clusters: %zu", cluster_indices.size());
    
        // 6. Circle fitting per cluster
        center_z0_cloud_->clear();
